...
if (isset($_GET["id"])) {
    ...
    $delete_query = mysqli_query($connection, "DELETE FROM returned WHERE ID ='$id'");
    if ($delete_query) {
        echo "
        <script>;
    ...
    <div class="pt-3 pr-40">
        <div class="ml-[150px]  bg-[#525289] w-[88vw] h-[97vh] rounded-[20px] p-8">
            ...
            <div class="h-[60px] rounded-xl bg-[#1D2041] p-4 grid grid-cols-1 lg:grid-cols-3">
                <div class="flex">
                    <h2 class="text-xl text-white">Returned Books</h2>

                </div>

                <div class=" -mt-1 flex">
                    <form action="" method="post">
                        <input type="search" class="h-9 w-60 rounded-md outline-none p-2" placeholder="Search by ID" id="myInput" onkeyup="mySearch()">
                    </form>
                </div>
            ...
            <div class="mt-10 flex items-center justify-center">
                <table id="myTable">
                    ...
                    <tbody>
                        ...
                        //selecting data from the database
                        $select_query = mysqli_query($connection, "SELECT * FROM `returned`");
